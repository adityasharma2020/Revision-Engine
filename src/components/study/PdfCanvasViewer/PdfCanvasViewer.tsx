...
function initialInkPreferences(): { tool: InkTool; colors: ToolColors; sizes: ToolSizes; advanced: AdvancedToolPreferences; presets: ToolPreset[] } {
  try {
    ...
    const size = (tool: InkTool, fallback: number) => Math.min(6, Math.max(1, Number(value.sizes?.[tool]) || (value.size ? legacySize : fallback)));
    ...
    const advanced: AdvancedToolPreferences = {
      ...
      highlighterOpacity: Math.min(.7, Math.max(.1, Number(value.advanced?.highlighterOpacity) || .3)),
    ...
    return {
      tool: ['pen', 'highlighter', 'line', 'eraser'].includes(value.tool ?? '') ? value.tool! : 'pen',
      ...
      advanced,
      presets: Array.isArray(value.presets) ? value.presets.slice(0, 8) : [],
    };
...
function AnnotationLayer({ pageNumber, rotation, annotations, editing, tool, color, size, advanced, onBegin, onExtend, onEnd, onErase }: {
  ...
  const updateEraserCursor = (point: PdfAnnotationPoint | undefined, visible: boolean) => {
    ...
    const screen = screenPoint(point, rotation); cursor.setAttribute('cx', String(screen.x * 1000)); cursor.setAttribute('cy', String(screen.y * 1000)); cursor.style.opacity = '1';
  };
...
function PdfPage({ document, pageNumber, width, zoom, rotation, register, annotations, hideNativeAnnotations, editing, tool, color, inkSize, advanced, onBegin, onExtend, onEnd, onErase, onNavigate }: {
  ...
  return <section ref={(node) => { hostRef.current = node; register(pageNumber, node); }} style={style} className={styles.page} data-pdf-page={pageNumber} aria-label={`Page ${pageNumber}`}>
    ...
    {(editing || annotations.length > 0) && <AnnotationLayer pageNumber={pageNumber} rotation={rotation} annotations={annotations} editing={editing} tool={tool} color={color} size={inkSize} advanced={advanced} onBegin={onBegin} onExtend={onExtend} onEnd={onEnd} onErase={onErase} />}
  </section>;
}
...
export function PdfCanvasViewer({ url, name, className, fileHandle, controlsInHeader = false, cloudAnnotations, onCloudAnnotationsChange }: PdfCanvasViewerProps) {
  ...
  const begin = (annotation: PdfInkAnnotation) => { drawingSnapshotRef.current = annotations; setPdfSaveDirty(true); if (!toolbarPinned) setToolbarCollapsed(true); setAnnotations((items) => [...items, annotation]); };
  const flushInk = () => {
  ...
  const discardEditableAnnotations = async () => {
    ...
    setAnnotations([]); setAutosaveState('saved'); setPdfSaveDirty(false);
  ...
  const swipeStart = (event: ReactPointerEvent<HTMLDivElement>) => { if (editing || event.pointerType === 'mouse' || (event.target as Element).closest(`.${styles.textLayer}`)) return; touchRef.current = { x: event.clientX, y: event.clientY }; };
  const swipeEnd = (event: ReactPointerEvent<HTMLDivElement>) => { const start = touchRef.current; touchRef.current = null; if (!start || editing) return; const dx = event.clientX - start.x; const dy = event.clientY - start.y; if (Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy) * 1.4) goTo(pageNumber + (dx < 0 ? 1 : -1)); };
  ...
  return <div className={cx(styles.reader, controlsInHeader && styles.controlsInHeader, className)} data-pdf-canvas-reader tabIndex={0} onKeyDown={keyboard}>
    {document && <>
      <div className={styles.controls} aria-label="PDF controls">
        ...
        <button type="button" className={editing ? styles.inkActive : ''} onClick={() => { setEditing((value) => !value); setToolbarCollapsed(false); setSearchOpen(false); }} aria-pressed={editing} aria-label={editing ? 'Editing mode active — switch to reading mode' : 'Reading mode active — switch to editing mode'} title={editing ? 'Editing mode · switch to reading' : 'Reading mode · switch to editing'}><Icon name={editing ? 'pencil' : 'book'} size={16} /></button>
        ...
        <div ref={documentMenuRef} className={styles.documentMenuHost}><button type="button" className={documentMenuOpen ? styles.searchActive : ''} onClick={() => setDocumentMenuOpen((open) => { const next = !open; if (next) setToolbarCollapsed(true); return next; })} aria-label="More PDF options"><Icon name="more" size={16} /></button>{documentMenuOpen && <div className={styles.documentMenu}>
          ...
          <button type="button" disabled={!annotations.length} className={styles.dangerMenuAction} onClick={() => { setDiscardDialogOpen(true); setDocumentMenuOpen(false); }}><Icon name="trash" size={15} /><span><strong>Discard editable annotations</strong><small>Remove all {annotations.length} local annotation{annotations.length === 1 ? '' : 's'}</small></span></button>
        </div>}</div>
      </div>
      ...
      {editing && !toolbarCollapsed && <div ref={inkToolbarRef} className={styles.inkToolbar} aria-label="Annotation tools">
        {(['pen', 'highlighter', 'line', 'eraser'] as const).map((value) => <button type="button" key={value} className={tool === value ? styles.toolActive : ''} onClick={() => { if (tool === value) setToolSettingsOpen((open) => !open); else { setTool(value); setToolSettingsOpen(false); } }} onDoubleClick={() => setToolSettingsOpen(true)} aria-label={value} title={tool === value ? 'Click again for tool settings' : value}><Icon name={value === 'pen' ? 'pencil' : value === 'highlighter' ? 'highlighter' : value === 'line' ? 'line' : 'eraser'} size={17} /></button>)}
        <span className={styles.inkDivider} /><input type="color" value={color} onChange={(event) => { if (tool !== 'eraser') setToolColors((current) => ({ ...current, [tool]: event.target.value })); }} aria-label="Ink colour" disabled={tool === 'eraser'} /><input type="range" min="1" max="6" step="1" value={inkSize} onChange={(event) => setToolSizes((current) => ({ ...current, [tool]: Number(event.target.value) }))} aria-label={`${tool} thickness`} title={`${tool} thickness: ${inkSize}`} />
      ...
      {editing && !toolbarCollapsed && toolSettingsOpen && <section ref={toolSettingsRef} className={styles.toolSettings} aria-label={`${tool} advanced settings`}>
        ...
        <label className={styles.toolRange}><span>Thickness <b>{inkSize}</b></span><input type="range" min="1" max="6" step="1" value={inkSize} onChange={(event) => setToolSizes((current) => ({ ...current, [tool]: Number(event.target.value) }))} /></label>
    ...
    <div ref={viewportRef} className={cx(styles.viewport, viewMode === 'page' && styles.pageMode, editing && styles.editViewport)} onPointerDown={swipeStart} onPointerUp={swipeEnd}>
      ...
      {!error && document && visiblePages.map((page) => <PdfPage key={page} document={document} pageNumber={page} width={width} zoom={zoom} rotation={rotation} register={(number, node) => { if (node) pageRefs.current.set(number, node); else pageRefs.current.delete(number); }} annotations={annotationsByPage.get(page) ?? []} hideNativeAnnotations={nativeInkImported} editing={editing} tool={tool} color={color} inkSize={inkSize} advanced={advanced} onBegin={begin} onExtend={extend} onEnd={end} onErase={erase} onNavigate={(destination, action) => void followLink(destination, action)} />)}
    </div>
    ...
    {discardDialogOpen && <div className={styles.saveBackdrop} role="presentation" onPointerDown={() => setDiscardDialogOpen(false)}><section className={cx(styles.saveDialog, styles.discardDialog)} role="alertdialog" aria-modal="true" aria-labelledby="discard-annotations-title" onPointerDown={(event) => event.stopPropagation()}>
      <header><span><Icon name="trash" size={18} /></span><div><strong id="discard-annotations-title">Discard editable annotations?</strong><small>This removes every pen, highlight and line currently stored by this app for this PDF.</small></div><button type="button" onClick={() => setDiscardDialogOpen(false)} aria-label="Close"><Icon name="close" size={16} /></button></header>
      <p>Marks already written into a previously saved PDF are permanent page content and cannot be removed by this action or the eraser.</p>
      <div className={styles.discardActions}><button type="button" onClick={() => setDiscardDialogOpen(false)}>Keep annotations</button><button type="button" onClick={() => void discardEditableAnnotations()}>Discard all</button></div>
    </section></div>}
  </div>;
}
