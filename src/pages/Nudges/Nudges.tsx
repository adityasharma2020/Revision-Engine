...
import { createNudge, getNudge, listNudges, recordNudgeFeedback, recordNudgeOpen, sendNudgeNow, setNudgeActive, updateNudge } from '../../services/nudges';
...
export function Nudges() {
  ...
  const sendNow = async (nudge: MemoryNudge) => {
    setSendingId(nudge.id); setMessage(null);
    try {
      const devices = await sendNudgeNow(nudge.id);
      setMessage(`“${nudge.title}” sent to ${devices} active ${devices === 1 ? 'device' : 'devices'}.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not send this nudge.'); }
  ...
  const testTarget = nudges.find((nudge) => nudge.id === testTargetId) ?? nudges[0];
  ...
  return (
    <Page narrow>
      ...
      <section className={styles.deliveryTest}>
        <div><span>Live delivery test</span><strong>Preview a real Memory Nudge on your devices</strong><small>This uses the actual content and push pipeline. It will not change the nudge schedule.</small></div>
        {testTarget ? <div className={styles.deliveryControls}><select aria-label='Nudge to test' value={testTarget.id} onChange={(event) => setTestTargetId(event.target.value)}>{nudges.map((nudge) => <option key={nudge.id} value={nudge.id}>{nudge.title}</option>)}</select><Button variant='primary' disabled={sendingId !== null} onClick={() => void sendNow(testTarget)}><Icon name='sparkle' size={15} />{sendingId === testTarget.id ? 'Sending…' : 'Send real test'}</Button></div> : <Button variant='primary' onClick={() => setEditing('new')}>Add your first nudge</Button>}
      </section>
      {message && <p className={styles.message} role='status'>{message}</p>}
      {selected && <ReviewCard nudge={selected} sending={sendingId === selected.id} onSend={() => sendNow(selected)} onDone={async (action, hours) => { await recordNudgeFeedback(selected, action, hours); setParams({}); setMessage(action === 'remembered' ? 'Marked remembered. Its interval is now longer.' : action === 'forgot' ? 'Marked for an earlier return.' : action === 'archive' ? 'Nudge archived.' : 'Nudge snoozed.'); await refresh(); }} />}
      ...
      {!loading && visible.length === 0 ? <EmptyState icon='sparkle' title='No memory nudges yet' description='Add one important fact, quote or mistake you never want to forget.' action={<Button variant='primary' onClick={() => setEditing('new')}>Create first nudge</Button>} /> : <div className={styles.grid}>{visible.map((nudge) => <article className={`${styles.card} ${!nudge.active ? styles.paused : ''}`} key={nudge.id}><header><span>{nudge.kind}</span><div aria-label={`Priority ${nudge.priority}`}>{'●'.repeat(nudge.priority)}<i>{'●'.repeat(5 - nudge.priority)}</i></div></header><h2>{nudge.title}</h2><p>{nudge.content}</p>{nudge.tags.length > 0 && <div className={styles.tags}>{nudge.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div>}<footer><small>{nudge.sendCount} deliveries · {nudge.cooldownHours}h cooldown</small><div><button className={styles.sendNow} disabled={sendingId !== null} onClick={() => void sendNow(nudge)}>{sendingId === nudge.id ? 'Sending…' : 'Send now'}</button><button onClick={() => { setSelected(nudge); setParams({ id: nudge.id }); }}>Review</button><button onClick={() => setEditing(nudge)}>Edit</button><button onClick={async () => { await setNudgeActive(nudge.id, !nudge.active); await refresh(); }}>{nudge.active ? 'Pause' : 'Resume'}</button></div></footer></article>)}</div>}
