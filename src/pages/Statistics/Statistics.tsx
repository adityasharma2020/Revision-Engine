...
export function Statistics() {
  const { quizResults, progress, annotations, questionAttemptLog } =
    useUserData();
  ...
  const [nudgeData, setNudgeData] = useState<NudgeAnalyticsData>({
    nudges: [],
    interactions: [],
  });
  ...
  useEffect(() => {
    if (authStatus !== "authenticated") {
      setNudgeData({ nudges: [], interactions: [] });
      return;
    }
    setNudgeLoading(true);
    setNudgeError("");
    void loadNudgeAnalytics()
      .then(setNudgeData)
      .catch((error) =>
        setNudgeError(
          error instanceof Error
            ? error.message
            : "Could not load Memory Nudge analytics."
        )
      )
  ...
  const { fQuiz, fProgress } = useMemo(() => {
    ...
    for (const [id, p] of Object.entries(progress)) {
      ...
      const attempts = Object.fromEntries(
        Object.entries(p.attempts).filter(
          ([, attempt]) =>
            attempt.attemptedAt >= cutoff && attempt.attemptedAt < end
        )
      );
      if (Object.keys(attempts).length > 0)
        filteredProgress[id] = { ...p, attempts };
    }
    return {
      fQuiz: quizResults.filter(
        (result) =>
          result.takenAt >= cutoff &&
          result.takenAt < end &&
          (selected.size === 0 || selected.has(result.chapterId))
      ),
  ...
  const activityDays = useMemo(() => {
    ...
    if (timeRange !== "all") {
      return Math.max(
        1,
        Math.floor((Date.now() - startOfRange(timeRange)) / 86_400_000) + 1
      );
    }
    const timestamps = [
    ...
    return Math.max(
      7,
      Math.ceil((Date.now() - first) / (24 * 60 * 60 * 1000)) + 1
    );
  ...
  const activity = useMemo(
    () =>
      dailyActivity(
        ...
        meta,
        activityDays,
        timeRange === "yesterday" ? endOfRange(timeRange) - 1 : Date.now()
      ),
  ...
  const dailyQuizResults = useMemo(() => {
    ...
    return quizResults
      .filter((result) => {
        ...
        if (
          result.purpose !== "daily-revision" &&
          result.chapterId !== "daily-revision"
        )
          return false;
        return (
          selected.size === 0 ||
          result.perQuestion?.some((item) => selected.has(item.chapterId ?? ""))
        );
      })
  ...
  const questionStats = useMemo(() => {
    ...
    for (const result of quizResults) {
      ...
      for (const item of result.perQuestion ?? []) {
        ...
        const current = stats.get(key) ?? {
          ...
          chapterId,
          statement: item.questionStatement ?? item.questionId,
          attempts: 0,
          correct: 0,
          skipped: 0,
          timeMs: 0,
    ...
    for (const attempt of questionAttemptLog) {
      ...
      const current = stats.get(key) ?? {
        ...
        statement: attempt.questionText ?? attempt.questionId,
        attempts: 0,
        correct: 0,
        skipped: 0,
        timeMs: 0,
      ...
      current.outcomes.push({
        ...
        value: attempt.correct ?? null,
      });
      current.tags = [
        ...new Set([...current.tags, ...(attempt.questionTags ?? [])]),
      ];
    ...
    const metric = (
      item: typeof stats extends Map<string, infer T> ? T : never
    ) => {
      const answered = item.attempts - item.skipped;
      const accuracy = answered
        ? Math.round((item.correct / answered) * 100)
        : 0;
      ...
      for (
        ...
        index >= 0 && chronological[index].value === true;
        index -= 1
      )
        streak += 1;
    ...
    return [...stats.values()]
      .filter((item) => {
        ...
        if (
          ...
          item.difficulty !== questionDifficulty
        )
          return false;
        if (
          questionSource === "pyq" &&
          !item.origin?.toUpperCase().startsWith("PYQ")
        )
          return false;
        if (
        ...
        if (
          questionView === "weak" &&
          !(values.accuracy < 60 || values.wrong >= 2)
        )
          return false;
        if (
          questionView === "mastered" &&
          !(values.accuracy >= 80 && values.streak >= 3)
        )
          return false;
        if (questionView === "skipped" && item.skipped === 0) return false;
        return (
          !query ||
          `${item.statement} ${item.questionId} ${
            meta[item.chapterId]?.title ?? ""
          } ${item.tags.join(" ")}`
            .toLowerCase()
            .includes(query)
        );
      ...
      .sort((a, b) => {
        ...
        if (questionSort === "wrong")
          return right.wrong - left.wrong || b.attempts - a.attempts;
        if (questionSort === "accuracy")
          return left.accuracy - right.accuracy || b.attempts - a.attempts;
        if (questionSort === "reviews")
          return b.reviews30d - a.reviews30d || b.attempts - a.attempts;
  ...
  const questionPageCount = Math.max(
    1,
    Math.ceil(questionStats.length / questionPageSize)
  );
  ...
  const hasAnyActivity =
    ...
    nudgeData.nudges.length > 0 ||
    nudgeData.interactions.length > 0;
  const scopeHasData =
    overview.answered > 0 ||
    overview.quizzes > 0 ||
    overview.chaptersRevised > 0;
          ...
          <section className={styles.filters} aria-label='Analytics filters'>
            <div className={styles.filterHead}>
              <div>
                ...
                <span>
                  {analyticsTab === "nudges"
                    ? "The date range applies to deliveries, opens and feedback."
                    : analyticsTab === "focus"
                    ? "Only sessions completed and confirmed in this period are included."
                    : "The range and chapter selection apply to everything below."}
                </span>
              </div>
              <div
            ...
            <>
              <div className={styles.primaryTiles}>
                ...
                <StatTile
                  ...
                  value={String(overview.answered)}
                />
                <StatTile
                  icon='clock'
                  label='Time studied'
                ...
                <section className={`${styles.card} ${styles.activityCard}`}>
                  ...
                  <BarChart
                    data={activity.map((d) => ({
                      label: d.label,
                      value: d.questions,
                      tooltip: `${new Date(d.at).toLocaleDateString(undefined, {
                        weekday: "short",
                        day: "numeric",
                        month: "short",
                        year: "numeric",
              ...
              {dailyQuizResults.length > 0 && (
                <section className={`${styles.card} ${styles.standaloneCard}`}>
                  <div className={styles.cardHead}>
                    <div>
                      <h2 className={styles.cardTitle}>
                        Daily Revision history
                      </h2>
                      <span className={styles.cardHint}>
                        Every completed attempt remains reviewable, including
                        reattempts
                      </span>
                    </div>
                    <span className={styles.dailyCompleteBadge}>
                      {dailyQuizResults.length}{" "}
                      {dailyQuizResults.length === 1 ? "attempt" : "attempts"}
                    </span>
                  </div>
                    ...
                    <table className={styles.table}>
                      <thead>
                        <tr>
                          ...
                          <th className={styles.num}>Questions</th>
                          <th className={styles.num}>Time</th>
                          <th aria-label='Review' />
                        </tr>
                      </thead>
                      <tbody>
                        {dailyQuizResults.slice(0, 30).map((result) => {
                          const date = result.dailyDateKey
                            ? new Date(`${result.dailyDateKey}T12:00:00`)
                            : new Date(result.takenAt);
                          const accuracy = result.answered
                            ? Math.round(
                                (result.correct / result.answered) * 100
                              )
                            : 0;
                          const resultDay =
                            result.dailyDateKey ??
                          ...
                          const sameDay = dailyQuizResults.filter(
                            (item) =>
                              (item.dailyDateKey ??
                          ...
                          const attemptNumber = sameDay.filter(
                            (item) => item.takenAt <= result.takenAt
                          ).length;
                          return (
                            <tr key={result.id}>
                              <td>
                              ...
                              <td className={styles.num}>
                                {humanizeDuration(result.durationMs)}
                              </td>
                              <td className={styles.num}>
                                <Link
                                  className={styles.reviewLink}
                                  to={Routes.quizResult(result.id)}
                                >
                                  Review
                                </Link>
                              </td>
                  ...
                  {dailyQuizResults.length > 30 && (
                    <p className={styles.tableNote}>
                      Showing the latest 30 of {dailyQuizResults.length} dated
                      Daily Revision attempts.
                    </p>
                  )}
                </section>
              )}
              ...
              {chapterStats.length > 1 && (
                <section className={`${styles.card} ${styles.standaloneCard}`}>
                  <div className={styles.cardHead}>
                    <div>
                      <h2 className={styles.cardTitle}>
                        Performance by original chapter
                      </h2>
                      <span className={styles.cardHint}>
                        Mixed and Daily Revision questions are credited to the
                        chapter they came from. Totals include every non-deleted
                        practice and quiz attempt.
                      </span>
                    </div>
                    <span className={styles.cardHint}>
                      ...
                      <thead>
                        <tr>
                          ...
                          <th
                            ...
                            title='Times questions from this chapter appeared across your attempts'
                          >
                            Questions shown
                          </th>
                          <th
                            className={styles.num}
                            title='Correct answers out of answered questions'
                          >
                            Your result
                          </th>
                          ...
                          <th
                            className={styles.num}
                            title='Unique questions seen out of all questions in this chapter'
                          >
                            Bank seen
                          </th>
                        </tr>
                      ...
                      <tbody>
                        {chapterStats.map((c) => (
                          <tr key={c.chapterId}>
                            ...
                            <td className={styles.num}>
                              <span className={styles.chapterResult}>
                                <strong>
                            ...
                            <td className={styles.num}>
                              {c.avgTimeMs
                                ? humanizeDuration(c.avgTimeMs)
                                : "—"}
                            </td>
                            <td className={styles.num}>
                              <span className={styles.chapterResult}>
                                <strong>
                                  {c.uniqueQuestions}/{c.totalQuestions}
                                </strong>
                                <small>{c.coverage}% coverage</small>
                              </span>
                            </td>
                          </tr>
                        ))}
...
function FocusAnalytics({
  ...
  const filtered = sessions.filter(
    (session) => session.completedAt >= cutoff && session.completedAt < end
  );
  const totalMs = filtered.reduce(
    (sum, session) => sum + session.creditedMs,
    0
  );
  const averageMs = filtered.length ? totalMs / filtered.length : 0;
  const reachedTarget = filtered.filter(
    (session) => (session.outcome ?? "completed") === "completed"
  ).length;
  ...
  for (const session of filtered) {
    ...
    item.value += Math.round(session.creditedMs / 60_000);
    item.count += 1;
    byDay.set(key, item);
  }
  ...
  if (!filtered.length)
    return (
      <EmptyState
        ...
        description={`Start a timer and any focused time you save will appear in ${TIME_RANGE_LABELS[
          range
        ].toLowerCase()} analytics.`}
      />
    );
  return (
    <section className={styles.focusAnalytics}>
      <div className={styles.primaryTiles}>
        <StatTile
          icon='clock'
          label='Total focus time'
          value={humanizeDuration(totalMs)}
          sublabel={TIME_RANGE_LABELS[range]}
        />
        <StatTile
          icon='check'
          label='Sessions tracked'
          value={String(filtered.length)}
          sublabel={`${reachedTarget} reached target`}
        />
        <StatTile
          icon='target'
          label='Average session'
          value={humanizeDuration(averageMs)}
          sublabel={`${focusStreak(sessions)} day current streak`}
        />
      </div>
      <section className={`${styles.card} ${styles.activityCard}`}>
        <div className={styles.cardHead}>
          <div>
            <h2 className={styles.cardTitle}>Focused minutes by day</h2>
            <span className={styles.cardHint}>
              Includes both completed targets and sessions ended early.
            </span>
          </div>
        </div>
        <BarChart
          data={chart.map((item) => ({
            ...
            value: item.value,
            tooltip: `${new Date(item.at).toLocaleDateString(undefined, {
              weekday: "long",
              day: "numeric",
              month: "short",
            })} · ${item.value} focused minutes · ${item.count} session${
              item.count === 1 ? "" : "s"
            }`,
          }))}
          valueSuffix=' min'
      ...
      <section className={`${styles.card} ${styles.standaloneCard}`}>
        <div className={styles.cardHead}>
          <div>
            ...
            <span className={styles.cardHint}>
              The target and actual focused time remain separate and
              transparent.
            </span>
          </div>
        </div>
          ...
          <table className={styles.table}>
            <thead>
              <tr>
                ...
                <th className={styles.num}>Focused</th>
                <th className={styles.num}>Paused</th>
              </tr>
            </thead>
            <tbody>
              {filtered
                ...
                .map((session) => (
                  <tr key={session.id}>
                    ...
                    <td className={styles.num}>
                      {humanizeDuration(session.targetMs)}
                    </td>
                    <td className={styles.num}>
                      {humanizeDuration(session.creditedMs)}
                    </td>
                    <td className={styles.num}>
                      {session.totalPausedMs
                        ? humanizeDuration(session.totalPausedMs)
                        : "—"}
                    </td>
                  </tr>
                ))}
...
function NudgeAnalytics({
  ...
  const productionDeliveries = interactions.filter(
    (item) => item.action === "delivered" && item.metadata.test !== true
  );
  const testDeliveries = interactions.filter(
    (item) => item.action === "delivered" && item.metadata.test === true
  );
  const opened = interactions.filter((item) => item.action === "opened");
  const remembered = interactions.filter(
    (item) => item.action === "remembered"
  );
  ...
  const recallRate = responses
    ? Math.round((remembered.length / responses) * 100)
    : 0;
  ...
  const ready = active.filter(
    (item) =>
      !item.nextEligibleAt ||
      new Date(item.nextEligibleAt).getTime() <= Date.now()
  );
  ...
  const categoryRows = kinds
    .map((kind) => {
      ...
      const matching = (item: NudgeInteractionRecord) =>
        byId.get(item.nudgeId)?.kind === kind;
      ...
      return {
        ...
        recall: decisions
          ? Math.round((rememberedCount / decisions) * 100)
          : null,
      };
    })
    .filter((row) => row.saved > 0 || row.deliveries > 0 || row.reviews > 0);
  const needsAttention = current
    .filter((item) => item.forgottenCount > 0)
    .sort(
      (a, b) =>
        b.forgottenCount -
          b.rememberedCount -
          (a.forgottenCount - a.rememberedCount) || b.priority - a.priority
    )
    .slice(0, 5);
  ...
  if (loading)
    return (
      <section
        className={`${styles.card} ${styles.nudgeLoading}`}
        aria-busy='true'
      >
        <span />
        <div>
          <h2 className={styles.cardTitle}>Loading Memory Nudge analytics</h2>
          <p className={styles.cardHint}>
            Calculating retention and delivery signals…
          </p>
        </div>
      </section>
    );
  if (error)
    return (
      <EmptyState
        icon='chart'
        title='Could not load nudge analytics'
        description={error}
      />
    );
  if (!data.nudges.length && !data.interactions.length)
    return (
      <EmptyState
        icon='sparkle'
        title='No Memory Nudge data yet'
        description='Add your first nudge and its delivery and retention signals will appear here.'
        action={
          <Link className={styles.reviewLink} to={Routes.nudges}>
            Create a nudge
          </Link>
        }
      />
  ...
  return (
    <section className={styles.nudgeAnalytics}>
      <div className={styles.primaryTiles}>
        <StatTile
          icon='sparkle'
          label='Saved nudges'
          value={String(current.length)}
          sublabel={`${active.length} active`}
        />
        <StatTile
          icon='share'
          label='Real deliveries'
          value={String(productionDeliveries.length)}
          sublabel={TIME_RANGE_LABELS[range]}
        />
        <StatTile
          icon='check'
          label='Recall signal'
          value={responses ? `${recallRate}%` : "—"}
          sublabel={
            responses
              ? `${remembered.length}/${responses} remembered`
              : "No feedback yet"
          }
        />
      </div>
      <div
        className={styles.secondaryMetrics}
        aria-label='Memory Nudge operational statistics'
      >
        <CompactMetric
      ...
      <div className={styles.nudgeGrid}>
        <section className={styles.card}>
          <div className={styles.cardHead}>
            <div>
              <h2 className={styles.cardTitle}>Retention by category</h2>
              <span className={styles.cardHint}>
                Use this to find content types that are being forgotten—not
                merely delivered.
              </span>
            </div>
          </div>
          {categoryRows.length ? (
            <div className={styles.tableWrap}>
              <table className={`${styles.table} ${styles.retentionTable}`}>
                <thead>
                  <tr>
                    <th>Type</th>
                    <th className={styles.num}>Saved</th>
                    <th className={styles.num}>Delivered</th>
                    <th className={styles.num}>Reviews</th>
                    <th className={styles.num}>Remembered</th>
                    <th className={styles.num}>Forgotten</th>
                ...
                <tbody>
                  {categoryRows.map((row) => (
                    <tr key={row.kind}>
                      <td className={styles.nudgeKind}>{row.kind}</td>
                      <td className={styles.num}>{row.saved}</td>
                      <td className={styles.num}>{row.deliveries}</td>
                      <td className={styles.num}>{row.reviews}</td>
                      <td className={styles.num}>{row.remembered}</td>
                      <td className={styles.num}>{row.forgotten}</td>
                      <td className={styles.num}>
                        {row.recall === null ? "—" : `${row.recall}%`}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <section className={styles.card}>
          <div className={styles.cardHead}>
            <div>
              ...
              <span className={styles.cardHint}>
                Items with recorded “forgot” feedback, ordered by the strongest
                difficulty signal.
              </span>
            </div>
          </div>
          {needsAttention.length ? (
            <ol className={styles.attentionList}>
              {needsAttention.map((nudge) => (
                <li key={nudge.id}>
                  <Link to={`${Routes.nudges}?id=${nudge.id}`}>
                    <span>
                    ...
                    <b>{nudge.forgottenCount} forgot</b>
                  </Link>
                </li>
              ))}
            </ol>
            ...
            <div className={styles.healthyState}>
              ...
              <span>No forgotten items recorded yet.</span>
            </div>
          )}
        </section>
      </div>
      ...
      <section className={`${styles.card} ${styles.standaloneCard}`}>
        <div className={styles.cardHead}>
          <div>
            <h2 className={styles.cardTitle}>Recent nudge activity</h2>
            <span className={styles.cardHint}>
              A transparent audit of delivery and review events. Test deliveries
              are clearly marked.
            </span>
          </div>
          <span className={styles.cardHint}>{TIME_RANGE_LABELS[range]}</span>
        </div>
        {recent.length ? (
          <div className={styles.tableWrap}>
            <table className={`${styles.table} ${styles.activityTable}`}>
              ...
              <tbody>
                {recent.map((item) => {
                  const nudge = byId.get(item.nudgeId);
                  return (
                    <tr key={item.id}>
                      <td data-label='Activity'>
                        ...
                        >
                          ...
                          {item.metadata.test === true ? " · test" : ""}
                        </span>
                      </td>
                      <td data-label='Nudge'>
                        {nudge ? (
                          <Link
                            className={styles.nudgeTitleLink}
                            to={`${Routes.nudges}?id=${nudge.id}`}
                          >
                            {nudge.title}
                          </Link>
                        ) : (
                      ...
                      <td data-label='When' className={styles.num}>
                        {new Intl.DateTimeFormat(undefined, {
                          day: "numeric",
                          month: "short",
                          hour: "numeric",
          ...
          <p className={styles.cardHint}>
            No nudge activity in this date range.
          </p>
        )}
      </section>
    </section>
    ...
    <section className={`${styles.card} ${styles.questionAnalytics}`}>
      <div className={styles.questionAnalyticsHead}>
        <div>
          ...
          <span className={styles.cardHint}>
            {total} matching practiced question{total === 1 ? "" : "s"} · 25 per
            page
          </span>
        </div>
        <label className={styles.questionSearch}>
          ...
          <input
            aria-label='Search question analytics'
            value={search}
            onChange={(event) => onSearch(event.target.value)}
            placeholder='Type words from a question…'
          />
        </label>
      </div>
      ...
      <div className={styles.questionFilters}>
        <fieldset className={styles.performanceFilter}>
            ...
            {(
              [
                [
                  "all",
                  "All practiced",
                  "Every question attempted in the selected period.",
                ],
                [
                  "weak",
                  "Needs attention",
                  "Below 60% accuracy or answered incorrectly at least twice.",
                ],
                [
                  "mastered",
                  "Mastered",
                  "At least 80% accuracy with three consecutive correct answers.",
                ],
          ...
          <small>
            {view === "weak"
              ? "Below 60% accuracy or wrong 2+ times."
              : view === "mastered"
              ? "80%+ accuracy and a 3-answer correct streak."
              : view === "skipped"
              ? "Skipped at least once."
              : "All questions you have practiced."}
          </small>
        </fieldset>
        <label>
          ...
          <small>Uses question-bank metadata.</small>
        </label>
        <label>
          ...
          <small>PYQ or practice material.</small>
        </label>
        <label>
          <span>Practice count</span>
          <select
            value={minAttempts}
            onChange={(event) => onMinAttempts(Number(event.target.value))}
          >
            {[1, 2, 3, 5, 10].map((value) => (
              <option key={value} value={value}>
                Attempted {value}+ time{value === 1 ? "" : "s"}
              </option>
            ))}
          </select>
          <small>Minimum recorded attempts.</small>
        </label>
        <label>
          <span>Order results</span>
          <select
            value={sort}
            onChange={(event) => onSort(event.target.value as QuestionSort)}
          >
            <option value='attempts'>Most attempted first</option>
            <option value='wrong'>Most incorrect first</option>
            <option value='accuracy'>Lowest accuracy first</option>
            <option value='reviews'>Most reviewed this month</option>
            <option value='recent'>Most recently reviewed</option>
          </select>
          <small>Changes order, not matches.</small>
        </label>
      </div>
      ...
      {items.length === 0 ? (
        ...
        <ol className={styles.questionList} start={(page - 1) * 25 + 1}>
          {items.map((item) => {
            ...
            const wrong = answered - item.correct;
            let streak = 0;
            const chronological = item.outcomes
              .slice()
              .sort((a, b) => a.at - b.at);
            for (
              let index = chronological.length - 1;
            ...
            return (
              <li
                key={`${item.chapterId}:${item.questionId}`}
              ...
              >
                <div className={styles.questionMain}>
                  <Link
                    ...
                    to={`${Routes.chapter(
                      item.chapterId
                    )}?tab=prelims#question-${encodeURIComponent(
                      item.questionId
                    )}`}
                  >
                    {item.statement}
                  ...
                  {item.tags.length > 0 && (
                    <div className={styles.tagList}>
                      ...
                      {item.tags.length > 6 && (
                        <small>+{item.tags.length - 6}</small>
                      )}
                    </div>
                  )}
                </div>
                <dl className={styles.questionMetrics}>
                  <div>
                    ...
                    <dd
                      className={
                        accuracy < 60 ? styles.metricDanger : undefined
                      }
                    >
                      {accuracy}%
                    </dd>
                  ...
                  <div>
                    <dt>Wrong</dt>
                    <dd className={wrong > 0 ? styles.metricDanger : undefined}>
                      {wrong}
                    </dd>
                  </div>
                  <div>
                    <dt>Correct streak</dt>
                    <dd>{streak}</dd>
                  </div>
                  <div>
                    <dt>Skipped</dt>
                  ...
                  <div>
                    ...
                    <dd>
                      {item.attempts
                        ? humanizeDuration(item.timeMs / item.attempts)
                        : "—"}
                    </dd>
                  </div>
                  <div>
                    <dt>Reviews · 30 days</dt>
                    <dd>{item.reviews30d}</dd>
                  </div>
                  <div>
                    <dt>Last reviewed</dt>
                    <dd>
      ...
      {total > 0 && (
        <nav
          ...
          aria-label='Question analytics pages'
        >
          <Button
            variant='secondary'
            size='sm'
          ...
          <span>
            Page <strong>{page}</strong> of {pageCount} · {total} questions
          </span>
          <Button
            variant='secondary'
            size='sm'
